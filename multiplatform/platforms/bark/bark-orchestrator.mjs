// bark-orchestrator.mjs — BRAIN for bark (model: apply).
//   node bark-orchestrator.mjs scan    -> discover actionable items, write state + queue
//   node bark-orchestrator.mjs record <key> <applied|posted|failed> [note]
// State: /app/data/bark-state.json  Queue: /app/data/bark-queue.json
// TODO(bark): implement discovery for the apply model. Until then this is a safe no-op.
import { readFileSync, writeFileSync, existsSync, mkdirSync } from "node:fs";
const DATA = "/app/data";
const STATE_FILE = DATA + "/bark-state.json";
const QUEUE_FILE = DATA + "/bark-queue.json";
...
async function scan(){
  const state = load();
  const queue = [];
  // TODO(bark): populate queue with undiscovered items (skip state.applied / recent state.failed).
  ...
  console.log(JSON.stringify({ platform:"bark", model:"apply", discovery:"not-implemented", queued:queue.length, applied:Object.keys(state.applied).length }));
}
