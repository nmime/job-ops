// epwk-orchestrator.mjs — BRAIN for epwk (model: apply).
//   node epwk-orchestrator.mjs scan    -> discover actionable items, write state + queue
//   node epwk-orchestrator.mjs record <key> <applied|posted|failed> [note]
// State: /app/data/epwk-state.json  Queue: /app/data/epwk-queue.json
// TODO(epwk): implement discovery for the apply model. Until then this is a safe no-op.
import { readFileSync, writeFileSync, existsSync, mkdirSync } from "node:fs";
const DATA = "/app/data";
const STATE_FILE = DATA + "/epwk-state.json";
const QUEUE_FILE = DATA + "/epwk-queue.json";
...
async function scan(){
  const state = load();
  const queue = [];
  // TODO(epwk): populate queue with undiscovered items (skip state.applied / recent state.failed).
  ...
  console.log(JSON.stringify({ platform:"epwk", model:"apply", discovery:"not-implemented", queued:queue.length, applied:Object.keys(state.applied).length }));
}
