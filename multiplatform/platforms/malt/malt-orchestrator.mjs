// malt-orchestrator.mjs — BRAIN for malt (model: apply).
//   node malt-orchestrator.mjs scan    -> discover actionable items, write state + queue
//   node malt-orchestrator.mjs record <key> <applied|posted|failed> [note]
// State: /app/data/malt-state.json  Queue: /app/data/malt-queue.json
// TODO(malt): implement discovery for the apply model. Until then this is a safe no-op.
import { readFileSync, writeFileSync, existsSync, mkdirSync } from "node:fs";
const DATA = "/app/data";
const STATE_FILE = DATA + "/malt-state.json";
const QUEUE_FILE = DATA + "/malt-queue.json";
...
async function scan(){
  const state = load();
  const queue = [];
  // TODO(malt): populate queue with undiscovered items (skip state.applied / recent state.failed).
  ...
  console.log(JSON.stringify({ platform:"malt", model:"apply", discovery:"not-implemented", queued:queue.length, applied:Object.keys(state.applied).length }));
}
