// kmong-orchestrator.mjs — BRAIN for kmong (model: apply).
//   node kmong-orchestrator.mjs scan    -> discover actionable items, write state + queue
//   node kmong-orchestrator.mjs record <key> <applied|posted|failed> [note]
// State: /app/data/kmong-state.json  Queue: /app/data/kmong-queue.json
// TODO(kmong): implement discovery for the apply model. Until then this is a safe no-op.
import { readFileSync, writeFileSync, existsSync, mkdirSync } from "node:fs";
const DATA = "/app/data";
const STATE_FILE = DATA + "/kmong-state.json";
const QUEUE_FILE = DATA + "/kmong-queue.json";
...
async function scan(){
  const state = load();
  const queue = [];
  // TODO(kmong): populate queue with undiscovered items (skip state.applied / recent state.failed).
  ...
  console.log(JSON.stringify({ platform:"kmong", model:"apply", discovery:"not-implemented", queued:queue.length, applied:Object.keys(state.applied).length }));
}
