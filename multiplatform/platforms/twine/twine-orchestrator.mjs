// twine-orchestrator.mjs — BRAIN for twine (model: apply).
//   node twine-orchestrator.mjs scan    -> discover actionable items, write state + queue
//   node twine-orchestrator.mjs record <key> <applied|posted|failed> [note]
// State: /app/data/twine-state.json  Queue: /app/data/twine-queue.json
// TODO(twine): implement discovery for the apply model. Until then this is a safe no-op.
import { readFileSync, writeFileSync, existsSync, mkdirSync } from "node:fs";
const DATA = "/app/data";
const STATE_FILE = DATA + "/twine-state.json";
const QUEUE_FILE = DATA + "/twine-queue.json";
...
async function scan(){
  const state = load();
  const queue = [];
  // TODO(twine): populate queue with undiscovered items (skip state.applied / recent state.failed).
  ...
  console.log(JSON.stringify({ platform:"twine", model:"apply", discovery:"not-implemented", queued:queue.length, applied:Object.keys(state.applied).length }));
}
