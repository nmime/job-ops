// ureed-orchestrator.mjs — BRAIN for ureed (model: apply).
//   node ureed-orchestrator.mjs scan    -> discover actionable items, write state + queue
//   node ureed-orchestrator.mjs record <key> <applied|posted|failed> [note]
// State: /app/data/ureed-state.json  Queue: /app/data/ureed-queue.json
// TODO(ureed): implement discovery for the apply model. Until then this is a safe no-op.
import { readFileSync, writeFileSync, existsSync, mkdirSync } from "node:fs";
const DATA = "/app/data";
const STATE_FILE = DATA + "/ureed-state.json";
const QUEUE_FILE = DATA + "/ureed-queue.json";
...
async function scan(){
  const state = load();
  const queue = [];
  // TODO(ureed): populate queue with undiscovered items (skip state.applied / recent state.failed).
  ...
  console.log(JSON.stringify({ platform:"ureed", model:"apply", discovery:"not-implemented", queued:queue.length, applied:Object.keys(state.applied).length }));
}
