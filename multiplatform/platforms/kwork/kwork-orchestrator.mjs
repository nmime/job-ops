// kwork-orchestrator.mjs — BRAIN for kwork (model: apply).
//   node kwork-orchestrator.mjs scan    -> discover actionable items, write state + queue
//   node kwork-orchestrator.mjs record <key> <applied|posted|failed> [note]
// State: /app/data/kwork-state.json  Queue: /app/data/kwork-queue.json
// TODO(kwork): implement discovery for the apply model. Until then this is a safe no-op.
import { readFileSync, writeFileSync, existsSync, mkdirSync } from "node:fs";
const DATA = "/app/data";
const STATE_FILE = DATA + "/kwork-state.json";
const QUEUE_FILE = DATA + "/kwork-queue.json";
...
async function scan(){
  const state = load();
  const queue = [];
  // TODO(kwork): populate queue with undiscovered items (skip state.applied / recent state.failed).
  ...
  console.log(JSON.stringify({ platform:"kwork", model:"apply", discovery:"not-implemented", queued:queue.length, applied:Object.keys(state.applied).length }));
}
