// arc-orchestrator.mjs — BRAIN for arc (model: apply).
//   node arc-orchestrator.mjs scan    -> discover actionable items, write state + queue
//   node arc-orchestrator.mjs record <key> <applied|posted|failed> [note]
// State: /app/data/arc-state.json  Queue: /app/data/arc-queue.json
// TODO(arc): implement discovery for the apply model. Until then this is a safe no-op.
import { readFileSync, writeFileSync, existsSync, mkdirSync } from "node:fs";
const DATA = "/app/data";
const STATE_FILE = DATA + "/arc-state.json";
const QUEUE_FILE = DATA + "/arc-queue.json";
...
async function scan(){
  const state = load();
  const queue = [];
  // TODO(arc): populate queue with undiscovered items (skip state.applied / recent state.failed).
  ...
  console.log(JSON.stringify({ platform:"arc", model:"apply", discovery:"not-implemented", queued:queue.length, applied:Object.keys(state.applied).length }));
}
