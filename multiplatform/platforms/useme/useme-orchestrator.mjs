// useme-orchestrator.mjs — BRAIN for useme (model: apply).
//   node useme-orchestrator.mjs scan    -> discover actionable items, write state + queue
//   node useme-orchestrator.mjs record <key> <applied|posted|failed> [note]
// State: /app/data/useme-state.json  Queue: /app/data/useme-queue.json
// TODO(useme): implement discovery for the apply model. Until then this is a safe no-op.
import { readFileSync, writeFileSync, existsSync, mkdirSync } from "node:fs";
const DATA = "/app/data";
const STATE_FILE = DATA + "/useme-state.json";
const QUEUE_FILE = DATA + "/useme-queue.json";
...
async function scan(){
  const state = load();
  const queue = [];
  // TODO(useme): populate queue with undiscovered items (skip state.applied / recent state.failed).
  ...
  console.log(JSON.stringify({ platform:"useme", model:"apply", discovery:"not-implemented", queued:queue.length, applied:Object.keys(state.applied).length }));
}
