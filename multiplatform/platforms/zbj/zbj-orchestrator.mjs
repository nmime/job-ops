// zbj-orchestrator.mjs — BRAIN for zbj (model: apply).
//   node zbj-orchestrator.mjs scan    -> discover actionable items, write state + queue
//   node zbj-orchestrator.mjs record <key> <applied|posted|failed> [note]
// State: /app/data/zbj-state.json  Queue: /app/data/zbj-queue.json
// TODO(zbj): implement discovery for the apply model. Until then this is a safe no-op.
import { readFileSync, writeFileSync, existsSync, mkdirSync } from "node:fs";
const DATA = "/app/data";
const STATE_FILE = DATA + "/zbj-state.json";
const QUEUE_FILE = DATA + "/zbj-queue.json";
...
async function scan(){
  const state = load();
  const queue = [];
  // TODO(zbj): populate queue with undiscovered items (skip state.applied / recent state.failed).
  ...
  console.log(JSON.stringify({ platform:"zbj", model:"apply", discovery:"not-implemented", queued:queue.length, applied:Object.keys(state.applied).length }));
}
