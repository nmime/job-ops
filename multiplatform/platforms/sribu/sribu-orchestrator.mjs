// sribu-orchestrator.mjs — BRAIN for sribu (model: apply).
//   node sribu-orchestrator.mjs scan    -> discover actionable items, write state + queue
//   node sribu-orchestrator.mjs record <key> <applied|posted|failed> [note]
// State: /app/data/sribu-state.json  Queue: /app/data/sribu-queue.json
// TODO(sribu): implement discovery for the apply model. Until then this is a safe no-op.
import { readFileSync, writeFileSync, existsSync, mkdirSync } from "node:fs";
const DATA = "/app/data";
const STATE_FILE = DATA + "/sribu-state.json";
const QUEUE_FILE = DATA + "/sribu-queue.json";
...
async function scan(){
  const state = load();
  const queue = [];
  // TODO(sribu): populate queue with undiscovered items (skip state.applied / recent state.failed).
  ...
  console.log(JSON.stringify({ platform:"sribu", model:"apply", discovery:"not-implemented", queued:queue.length, applied:Object.keys(state.applied).length }));
}
