// guru-orchestrator.mjs — BRAIN for guru (model: apply).
//   node guru-orchestrator.mjs scan    -> discover actionable items, write state + queue
//   node guru-orchestrator.mjs record <key> <applied|posted|failed> [note]
// State: /app/data/guru-state.json  Queue: /app/data/guru-queue.json
// TODO(guru): implement discovery for the apply model. Until then this is a safe no-op.
import { readFileSync, writeFileSync, existsSync, mkdirSync } from "node:fs";
const DATA = "/app/data";
const STATE_FILE = DATA + "/guru-state.json";
const QUEUE_FILE = DATA + "/guru-queue.json";
...
async function scan(){
  const state = load();
  const queue = [];
  // TODO(guru): populate queue with undiscovered items (skip state.applied / recent state.failed).
  ...
  console.log(JSON.stringify({ platform:"guru", model:"apply", discovery:"not-implemented", queued:queue.length, applied:Object.keys(state.applied).length }));
}
