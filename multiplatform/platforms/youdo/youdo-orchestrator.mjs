// youdo-orchestrator.mjs — BRAIN for youdo (model: apply).
//   node youdo-orchestrator.mjs scan    -> discover actionable items, write state + queue
//   node youdo-orchestrator.mjs record <key> <applied|posted|failed> [note]
// State: /app/data/youdo-state.json  Queue: /app/data/youdo-queue.json
// TODO(youdo): implement discovery for the apply model. Until then this is a safe no-op.
import { readFileSync, writeFileSync, existsSync, mkdirSync } from "node:fs";
const DATA = "/app/data";
const STATE_FILE = DATA + "/youdo-state.json";
const QUEUE_FILE = DATA + "/youdo-queue.json";
...
async function scan(){
  const state = load();
  const queue = [];
  // TODO(youdo): populate queue with undiscovered items (skip state.applied / recent state.failed).
  ...
  console.log(JSON.stringify({ platform:"youdo", model:"apply", discovery:"not-implemented", queued:queue.length, applied:Object.keys(state.applied).length }));
}
