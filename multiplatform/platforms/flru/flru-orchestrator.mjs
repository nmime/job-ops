// flru-orchestrator.mjs — BRAIN for flru (model: apply).
//   node flru-orchestrator.mjs scan    -> discover actionable items, write state + queue
//   node flru-orchestrator.mjs record <key> <applied|posted|failed> [note]
// State: /app/data/flru-state.json  Queue: /app/data/flru-queue.json
// TODO(flru): implement discovery for the apply model. Until then this is a safe no-op.
import { readFileSync, writeFileSync, existsSync, mkdirSync } from "node:fs";
const DATA = "/app/data";
const STATE_FILE = DATA + "/flru-state.json";
const QUEUE_FILE = DATA + "/flru-queue.json";
...
async function scan(){
  const state = load();
  const queue = [];
  // TODO(flru): populate queue with undiscovered items (skip state.applied / recent state.failed).
  ...
  console.log(JSON.stringify({ platform:"flru", model:"apply", discovery:"not-implemented", queued:queue.length, applied:Object.keys(state.applied).length }));
}
