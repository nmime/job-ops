...
export async function importDesignResumeFromFile(
  ...
  // Fork feature: when no LLM is configured we can still import a DOCX using
  // deterministic local text extraction instead of failing the whole import.
  const aiUnavailable =
    !provider || (providerRequiresApiKey(provider) && !runtime.apiKey);

  if (aiUnavailable && mediaType === DOCX_MIME) {
    const documentText = await extractDocxText(decoded);
    if (documentText) {
