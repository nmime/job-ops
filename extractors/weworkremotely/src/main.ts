...
const PLATFORM = "weworkremotely" as const;
const ENV_PREFIX = "JOBOPS_FREELANCE_WEWORKREMOTELY";
...
// ---------------------------------------------------------------------------
//
// We Work Remotely is a public board: there is no platform account and no
// apply form on the board side. Each listing links to the employer's ATS
// (Greenhouse, Lever, Workday, ...). This adapter:
//
//   1. re-resolves the board listing for ctx.gigId by re-fetching the same
//      public RSS feeds the finder uses and recomputing the dedup hash
//      (kept in sync with
//      orchestrator/src/server/services/freelance/dedupe.ts);
//   2. opens the listing in a real Playwright browser (stealth init script)
//      and resolves the employer apply target: the listing URL itself when
//      it is already an ATS page, otherwise the external apply link or the
//      apply button found on the listing page;
...
// which happens exclusively via
// JOBOPS_FREELANCE_WEWORKREMOTELY_APPLY_ENABLED=true. No credentials are
// needed (boards have no accounts).
...
/**
 * Find the employer's external apply URL on a board listing page: an ATS
...
/** Apply controls on the board listing page itself (if it is not an ATS). */
...
/**
 * We Work Remotely apply adapter.
 *
 * GUARDED: ctx.dryRun is forced true by the orchestrator unless
 * JOBOPS_FREELANCE_WEWORKREMOTELY_APPLY_ENABLED=true. No credentials exist
 * for this platform (public board) — the submit path drives the employer's
 * ATS form in a real browser and only reports "submitted" on a confirmed
 * success signal.
 */
export async function applyToWwrGig(
  ...
  try {
    ...
    if (!posting) {
      return {
        ...
        mode: "submit",
        status: "drafted",
        error: `${PLATFORM}: gig ${ctx.gigId} not found on the We Work Remotely feeds (listing may be closed) — cannot open its employer apply page`,
      };
    }
      ...
      if (!ATS_PATTERN.test(posting.gigUrl)) {
        ...
        if (!reached) {
          return {
            ...
            mode: "submit",
            status: "drafted",
            error: `${PLATFORM}: no employer application link found on listing ${posting.gigUrl}`,
          };
        }
      }
