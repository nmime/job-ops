 ...
 * Upwork finder.
 *
 * Upwork exposes no credential-free public listing API. A real finder needs
 * one of:
 *   - UPWORK_API_KEY   (official API / OAuth token)
 *   - UPWORK_COOKIE    (authenticated session cookie)
 *
...
/**
 * Upwork apply adapter.
 *
 * GUARDED: ctx.dryRun is forced true by the orchestrator unless
 * JOBOPS_FREELANCE_UPWORK_APPLY_ENABLED=true. Never submits real money-bearing
