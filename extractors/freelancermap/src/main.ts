...
/**
 * freelancermap (DE) finder.
 *
 * freelancermap (DE) exposes no credential-free public listing API. A real finder needs
 * one of:
 *   - FREELANCERMAP_API_KEY   (official API / OAuth token)
 *   - FREELANCERMAP_COOKIE    (authenticated session cookie)
 *
...
/**
 * freelancermap (DE) apply adapter.
 *
 * GUARDED: ctx.dryRun is forced true by the orchestrator unless
 * JOBOPS_FREELANCE_FREELANCERMAP_APPLY_ENABLED=true. Never submits real money-bearing
