...
/**
 * PeoplePerHour finder.
 *
 * PeoplePerHour exposes no credential-free public listing API. A real finder needs
 * one of:
 *   - PEOPLEPERHOUR_API_KEY   (official API / OAuth token)
 *   - PEOPLEPERHOUR_COOKIE    (authenticated session cookie)
 *
...
/**
 * PeoplePerHour apply adapter.
 *
 * GUARDED: ctx.dryRun is forced true by the orchestrator unless
 * JOBOPS_FREELANCE_PEOPLEPERHOUR_APPLY_ENABLED=true. Never submits real money-bearing
