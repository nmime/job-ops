 ...
 * Fiverr finder.
 *
 * Fiverr exposes no credential-free public listing API. A real finder needs
 * one of:
 *   - FIVERR_API_KEY   (official API / OAuth token)
 *   - FIVERR_COOKIE    (authenticated session cookie)
 *
...
/**
 * Fiverr apply adapter.
 *
 * GUARDED: ctx.dryRun is forced true by the orchestrator unless
 * JOBOPS_FREELANCE_FIVERR_APPLY_ENABLED=true. Never submits real money-bearing
