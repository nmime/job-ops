...
/**
 * Wellfound (AngelList) finder.
 *
 * Wellfound (AngelList) exposes no credential-free public listing API. A real finder needs
 * one of:
 *   - WELLFOUND_API_KEY   (official API / OAuth token)
 *   - WELLFOUND_COOKIE    (authenticated session cookie)
 *
...
/**
 * Wellfound (AngelList) apply adapter.
 *
 * GUARDED: ctx.dryRun is forced true by the orchestrator unless
 * JOBOPS_FREELANCE_WELLFOUND_APPLY_ENABLED=true. Never submits real money-bearing
